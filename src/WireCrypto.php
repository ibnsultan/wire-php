...
class WireCrypto
{
    ...
    public static function parse_master_key($encryption_master_key_base64): string
    {
        if (!function_exists('sodium_crypto_secretbox')) {
            throw new WireException('To use end to end encryption, you must either be using PHP 7.2 or greater or have installed the libsodium-php extension for php < 7.2.');
        }

        if ($encryption_master_key_base64 !== '') {
            $decoded_key = base64_decode($encryption_master_key_base64, true);
            if ($decoded_key === false) {
                throw new WireException('encryption_master_key_base64 must be a valid base64 string');
            }

            if (strlen($decoded_key) !== SODIUM_CRYPTO_SECRETBOX_KEYBYTES) {
                throw new WireException('encryption_master_key_base64 must encode a key which is 32 bytes long');
            }
    ...
    /**
    * Initialises a WireCrypto instance.
     *
    ...
    * @throws WireException
    ...
    public function decrypt_event(object $event): object
    {
        ...
        if (!$decrypted_payload) {
            throw new WireException('Decryption of the payload failed. Wrong key?');
        }
    ...
    {
        if (!self::is_encrypted_channel($channel)) {
            throw new WireException('You must specify a channel of the form private-encrypted-* for E2E encryption. Got ' . $channel);
        }
    ...
    {
        if (!self::is_encrypted_channel($channel)) {
            throw new WireException('Cannot encrypt plaintext for a channel that is not of the form private-encrypted-*. Got ' . $channel);
        }
    ...
    private function parse_encrypted_message(string $payload): object
    {
        ...
        if ($decoded_payload->ciphertext === '' || strlen($decoded_payload->nonce) !== SODIUM_CRYPTO_SECRETBOX_NONCEBYTES) {
            throw new WireException('Received a payload that cannot be parsed.');
        }
