...
class Wire implements LoggerAwareInterface, WireInterface
{
    ...
    * @var null|WireCrypto
    ...
    /**
    * Initializes a new Wire instance with key, secret, app ID and channel.
     *
     * @param string $auth_key
     * @param string $secret
     * @param string $app_id
     * @param array $options  [optional]
    *                         Options to configure the Wire instance.
     *                         scheme - e.g. http or https
    *                         host - the API host. No trailing forward slash.
     *                         port - the http port
     ...
     *
    * @throws WireException Throws exception if any required dependencies are missing
     */
    public function __construct(string $auth_key, string $secret, string $app_id, array $options = [], ?ClientInterface $client = null)
    {
        ...
        if ($options['encryption_master_key_base64'] !== '') {
            $parsedKey = WireCrypto::parse_master_key(
                $options['encryption_master_key_base64']
            );
            $this->crypto = new WireCrypto($parsedKey);
        }
    ...
    private function log(string $msg, array $context = [], string $level = LogLevel::DEBUG): void
    {
        ...
        $msg = sprintf('Wire: %s: %s', strtoupper($level), $msg);
        $replacement = [];
     ...
     *
     * @throws WireException If any required dependencies are missing
     */
    private function check_compatibility(): void
    {
        if (!extension_loaded('json')) {
            throw new WireException('The Wire library requires the PHP JSON module. Please ensure it is installed');
        }

        if (!in_array('sha256', hash_algos(), true)) {
            throw new WireException('SHA256 appears to be unsupported - make sure you have support for it, or upgrade your version of PHP.');
        }
    }
     ...
     *
    * @throws WireException If $channels is too big or any channel is invalid
     */
    private function validate_channels(array $channels): void
    {
        if (count($channels) > 100) {
            throw new WireException('An event can be triggered on a maximum of 100 channels in a single call.');
        }
     ...
     *
    * @throws WireException If $channel is invalid
     */
    private function validate_channel(string $channel): void
    {
        if (!preg_match('/\A#?[-a-zA-Z0-9_=@,.;]+\z/', $channel)) {
            throw new WireException('Invalid channel name ' . $channel);
        }
    }
    ...
    * @throws WireException If $socket_id is invalid
     ...
     *
    * @throws WireException If $user_id is invalid
     */
    private function validate_user_id(string $user_id): void
    {
        if ($user_id === null || empty($user_id)) {
            throw new WireException('Invalid user id ' . $user_id);
        }
    }
    ...
    public function make_request($channels, string $event, $data, array $params = [], bool $already_encoded = false): Request
    {
        ...
        foreach ($channels as $chan) {
            if (WireCrypto::is_encrypted_channel($chan)) {
        ...
        if ($has_encrypted_channel) {
            if (count($channels) > 1) {
                ...
                throw new WireException('You cannot trigger to multiple channels when using encrypted channels');
            } else {
                try {
                    ...
                    throw new WireException('Data encoding error.');
                }
            }
        } else {
            try {
        ...
        $headers = [
            ...
            'X-Wire-Library' => 'wire-http-php ' . self::$VERSION,
            'X-Pusher-Library' => 'wire-http-php ' . self::$VERSION
        ];
    ...
    * @throws WireException Throws WireException if $channels is an array of size 101 or above or $socket_id is invalid
     */
    ...
    * @throws WireException
    ...
    public function make_batch_request(array $batch = [], bool $already_encoded = false): Request
    {
        foreach ($batch as $key => $event) {
            ...
            if (WireCrypto::is_encrypted_channel($event['channel'])) {
    ...
    * @throws WireException     If $user_id is invalid
     ...
     * @param string $user_id
     *
    * @throws WireException     If $userId is invalid
     *
    ...
    * @param array  $params      API params (see the REST API documentation)
    ...
    * @throws WireException
     *
    * @return mixed See Wire API docs
     */
    public function get(string $path, array $params = [], $associative = false)
    {
        ...
        try {
            ...
            throw new WireException('Data decoding error.');
        }
     ...
     *
     * @param string $path        Path excluding /apps/APP_ID
    * @param mixed  $body        Request payload (see the REST API documentation)
    * @param array  $params      API params (see the REST API documentation)
     *
     * @throws ApiErrorException Throws ApiErrorException if the Channels HTTP API responds with an error
     * @throws GuzzleException
    ...
    * @throws WireException Throws exception if $channel is invalid or above or $socket_id is invalid
     */
    ...
    * @throws WireException Throws exception if $channel is invalid or above or $socket_id is invalid
    ...
    public function authorizeChannel(string $channel, string $socket_id, ?string $custom_data = null): string
    {
        ...
        if (WireCrypto::is_encrypted_channel($channel)) {
            if (!is_null($this->crypto)) {
                $signature['shared_secret'] = base64_encode($this->crypto->generate_shared_secret($channel));
            } else {
                throw new WireException('You must specify an encryption master key to authorize an encrypted channel');
            }
        }
    ...
    * @throws WireException Throws exception if $channel is invalid or above or $socket_id is invalid
    ...
    /**
    * Verify that a webhook actually came from Wire, decrypts any encrypted events, and marshals them into a PHP object.
     *
    ...
    * @throws WireException
    ...
    /**
    * Verify that a given Wire signature is valid.
     *
     * @param array  $headers an array of headers from the request (for example, from getallheaders())
     * @param string $body    the body of the request (for example, from file_get_contents('php://input'))
     *
     * @throws WireException if signature is incorrect.
     */
    public function verifySignature(array $headers, string $body): void
    {
        $wireKey = $headers['X-Wire-Key'] ?? $headers['X-Pusher-Key'] ?? null;
        $wireSignature = $headers['X-Wire-Signature'] ?? $headers['X-Pusher-Signature'] ?? null;

        if ($wireKey === $this->settings['auth_key']) {
            $expected = hash_hmac('sha256', $body, $this->settings['secret']);
            if ($expected === $wireSignature) {
                return;
            }
        }

        throw new WireException(sprintf('Received webhook with invalid signature: got %s.', $wireSignature));
    }
    ...
    * @throws WireException
    ...
    private function make_event(array $channels, string $event, $data, array $params = [], ?string $info = null, bool $already_encoded = false): array
    {
        ...
        if ($has_encrypted_channel) {
            if (WireCrypto::has_mixed_channels($channels)) {
                throw new WireException('You cannot trigger to encrypted and non-encrypted channels at the same time');
            } else {
                try {
                    $data_encoded = $this->crypto->encrypt_payload(
                        $channels[0],
    ...
    * @throws WireException
    ...
    private function validate_user_data(array $user_data): void
    {
        if (is_null($user_data)) {
            throw new WireException('user_data is null');
        }
        if (!array_key_exists('id', $user_data)) {
            throw new WireException('user_data has no id field');
        }
